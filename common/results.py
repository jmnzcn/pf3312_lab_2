...
from common.paths import project_root, results_dir
...
def _raw_dir() -> Path:
    return _results_path() / "raw"


def _archive_dir() -> Path:
    return _results_path() / "archive"
...
def append_results_csv(category: str, results: Iterable[BenchmarkResult]) -> Path:
    ...
    ensure_dirs()
    csv_path = _results_path() / f"{category}_results.csv"
...
def dump_results_json(category: str, provider: str, results: Iterable[BenchmarkResult]) -> Path:
    """JSON completo por proveedor en results/raw/ (sin truncar previews)."""
    ensure_dirs()
    out_path = _raw_dir() / f"{category}_{provider}.json"
...
def archive_results_csv() -> list[Path]:
    ...
    for category in ("llm", "stt", "tts"):
        src = _results_path() / f"{category}_results.csv"
        if not src.exists():
            continue
        dst = _archive_dir() / f"{category}_results_{safe}.csv"
        src.replace(dst)
        moved.append(dst)
    e2e_src = _results_path() / "e2e_results.csv"
    if e2e_src.exists():
        e2e_dst = _archive_dir() / f"e2e_results_{safe}.csv"
        e2e_src.replace(e2e_dst)
        moved.append(e2e_dst)
    return moved
